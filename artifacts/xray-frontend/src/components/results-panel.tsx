    ...
    >
      ...
      <div className="space-y-5 flex flex-col">
        ...
        <motion.div
          ...
          className={cn(
            "p-8 rounded-2xl border-2 flex flex-col items-center text-center space-y-3 shadow-sm",
            styles.container
          )}
        >
        ...
        >
          ...
          <div className="bg-muted/50 p-4 rounded-xl border border-border/50">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              {result.class_info.recommendation}
            </p>
          </div>
          ...
          <div className="pt-2 space-y-4">
            ...
            <div className="space-y-3">
              {result.all_scores.map((score, index) => {
                const itemStyle =
                  CLASS_STYLES[score.label.toLowerCase()] || CLASS_STYLES.healthy;
                const isTop = index === 0;
                return (
                  <div
                    key={score.label}
                    className="space-y-1.5"
                    data-testid={`bar-probability-${score.label}`}
                  >
